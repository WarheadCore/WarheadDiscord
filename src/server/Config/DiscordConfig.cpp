...
DiscordConfig* DiscordConfig::instance()
{
...
void DiscordConfig::AddOption(std::string_view optionName, Optional<std::string> def /*= std::nullopt*/)
{
    AddOption<std::string>(optionName, def);
}

void DiscordConfig::AddOption(std::initializer_list<std::string> const& optionList)
{
...
WH_SERVER_API T DiscordConfig::GetOption(std::string_view optionName, Optional<T> def /*= std::nullopt*/)
{
    ...
    if (itr == _configOptions.end())
    {
        LOG_CRIT("server.loading", "> DiscordConfig: option ({}) is not exists. Returned ({})", optionName, defStr);
    ...
    if (!result)
    {
        LOG_ERROR("server.loading", "> DiscordConfig: Bad value defined for '{}', use '{}' instead", optionName, defStr);
        return GetDefaultValue<T>();
    }
...
WH_SERVER_API void DiscordConfig::SetOption(std::string_view optionName, T value)
{
    ...
    if (itr == _configOptions.end())
    {
        LOG_ERROR("server.loading", "> DiscordConfig: option ({}) is not exists", optionName);
        return;
    }
